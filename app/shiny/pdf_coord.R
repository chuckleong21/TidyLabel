get_pdf_version <- function(file) {
  ...
  v <- v[nchar(v) != 0]
  version <- version_list[which(map_lgl(map(version_list, \(version) str_detect(v, version)), any))]
  ...
  if(n == 1) {
    ...
    tax_code <- switch (version,
                        ...
                        "7816168667" = list(
                          ...
                          map(seq_len(3), \(i) {
                            top = 620.72355; left = 61.85495; bottom = 711.72469; right = 90.59215
                            x = 257.8; y = 116.2
                            case_match(i, 
                                       1 ~ list(c(top, left, bottom, right)), 
                                       2 ~ list(c(top, left + x, bottom, right + x)), 
    ...
    tax <- switch (version,
                   ...
                   "7816168667" = list(
                     ...
                     map(seq_len(3), \(i) {
                       top = 617.8498; left = 234.2782; bottom = 712.6826; right = 306.1212
                       x = 257.8; y = 116.2
                       case_match(i, 
                                  1 ~ list(c(top, left, bottom, right)),
                                  2 ~ list(c(top, left + x, bottom, right + x)), 
