...
tidyTaxServer <- function(id, i18n, version) {
  moduleServer(id, function(input, output, session) {
    ...
    pdf_status_modal <- function() {
      ...
      status <- pdf_status()
      modalDialog(
        title = ifelse(preset == "auto", i18n$translate("自动检测结果"), i18n$translate("版本检测结果")),
        footer = tagList(
          modalButton("OK")
        ), 
        tagList(
            switch(status, 
                   "success" = span(i18n$translate("匹配"), style = "color:#198754"),
                   "warning" = span(i18n$translate("不匹配"), style = "color:#ffc107"), 
                   "error" = span(i18n$translate("缺失"), style = "color:dc3545")),
            br(),
            span(paste0(i18n$translate("检测到版本"), ":" , version())),
            if(preset != "auto") div(br(), span(paste0(i18n$translate("设置版本"), ":", preset))),
            if(status == "error" && preset == "auto") div(br(), span(paste0(i18n$translate("现有版本"), ":", version_list[-1]))),
            if(status != "success") div(br(), span(paste0(i18n$translate("版本不匹配"), "/", 
                                                          i18n$translate("版本缺失"), 
                                                          ifelse(session$userData$lang() == "en", " ", ""), 
                                                          i18n$translate("将导致结果出错"))))
        )
      )
    }
